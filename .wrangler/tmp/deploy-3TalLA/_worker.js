...
// functions/get-stripe-config.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var get_stripe_config_default = {
  async fetch(request, env2) {
    ...
    const publishableKey = env2.STRIPE_PUBLISHABLE_KEY;
    if (!publishableKey) {
      return new Response(
...
__name(validateAndPriceCart, "validateAndPriceCart");
var create_payment_intent_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, {
        headers: {
    ...
    const stripeSecret = env2.STRIPE_SECRET_KEY;
    if (!stripeSecret) {
      return new Response(
    ...
    try {
      ...
      console.error("Error creating payment intent:", error3);
      return new Response(
    ...
    return __awaiter(this, arguments, void 0, function* (functionName, options = {}) {
      ...
      try {
        ...
        if (error3 instanceof Error && error3.name === "AbortError") {
          return { data: null, error: new FunctionsFetchError(error3) };
        }
        return {
          data: null,
          error: error3,
...
async function assignAccount(env2, productId, plan, customerEmail) {
  const supabase = getSupabase(env2);
  console.log(`Searching for account: product_id="${productId}", plan="${plan}"`);
  const { data: accounts, error: fetchError } = await supabase.from("accounts").select("*").eq("product_id", productId).eq("plan", plan).eq("status", "available").order("created_at", { ascending: true }).order("id", { ascending: true }).limit(1);
  if (fetchError)
  ...
  if (account.chatgpt_password)
    result.chatgptPassword = account.chatgpt_password;
  if (account.chatgpt_code)
    result.chatgptCode = account.chatgpt_code;
  return result;
}
__name(assignAccount, "assignAccount");
var stripe_webhook_default = {
  async fetch(request, env2) {
    if (request.method !== "POST") {
      return new Response("Method Not Allowed", { status: 405 });
    }
    ...
    try {
        ...
        case "payment_intent.succeeded": {
          ...
          if (pi.metadata?.type === "balance_topup") {
            try {
              ...
              console.error("Error processing balance top-up:", error3);
            }
            break;
          }
          ...
          for (const item of cart) {
            ...
            for (let i = 0; i < qty; i++) {
              try {
                const credentials = await assignAccount(env2, item.pid, item.plan, customerEmail);
                const itemCreds = {
                  email: credentials.email,
                  password: credentials.password
                };
                if (credentials.chatgptPassword)
                  itemCreds.chatgptPassword = credentials.chatgptPassword;
                if (credentials.chatgptCode)
                  itemCreds.chatgptCode = credentials.chatgptCode;
                orderItems.push({
                  pid: item.pid,
                  name: item.pid,
                  plan: item.plan,
      ...
      console.error("Webhook handler error:", error3);
      return new Response(
        JSON.stringify({ error: error3.message }),
        { status: 500, headers: { "Content-Type": "application/json" } }
      );
...
var get_orders_default = {
  async fetch(request, env2) {
    ...
    try {
      ...
      const { data: orders, error: error3 } = await query;
      if (error3) {
...
// functions/get-stock.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase3(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_ANON_KEY;
  if (!url || !key) {
    throw new Error("Missing Supabase credentials");
...
__name(getSupabase3, "getSupabase");
var get_stock_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, {
        headers: {
    ...
    try {
      const supabase = getSupabase3(env2);
      const { data: stock, error: error3 } = await supabase.from("accounts").select("product_id, plan").eq("status", "available");
      if (error3) {
        console.error("Supabase error:", error3);
        return new Response(
          JSON.stringify({
      ...
      console.error("Error fetching stock:", error3);
      return new Response(
        JSON.stringify({
...
// functions/debug-netflix-accounts.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase4(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_ANON_KEY;
  if (!url || !key) {
    throw new Error("Missing Supabase credentials");
...
__name(getSupabase4, "getSupabase");
var debug_netflix_accounts_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, {
        headers: {
    ...
    try {
      const supabase = getSupabase4(env2);
      const { data: accounts, error: error3 } = await supabase.from("accounts").select("id, product_id, plan, status, created_at").or("product_id.ilike.%netflix%,plan.ilike.%netflix%").order("created_at", { ascending: false }).limit(50);
      if (error3) {
        return new Response(
          JSON.stringify({ error: error3.message }),
          {
...
// functions/crypto-now-create.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var PRICES_USD = {
  boosts: { "3 Months": 5.25 },
  netflix: { "1 Month": 1.5, "3 Months": 3.5, "6 Months": 6, "12 Months": 11, "Lifetime": 1.1, "FA": 8.5 },
  ...
  prime: { "1 Month": 1.8, "3 Months": 3.4, "6 Months": 5.8, "12 Months": 10.64, "Lifetime": 14 },
  hbomax: { "1 Month": 1.4, "3 Months": 3.1, "6 Months": 4.7, "12 Months": 9.8, "Lifetime": 13 },
  nordvpn: { "1 Month": 0.85, "3 Months": 2, "6 Months": 3.6, "12 Months": 6.9, "Lifetime": 1.46 },
  crunchy: { "1 Month": 0.9, "3 Months": 2.1, "6 Months": 3.8, "12 Months": 7.2, "Lifetime": 1.05 },
  nitro: { "Boost 1m": 4, "Boost 1 Year": 15.17, "Basic 1m": 1.35 },
  discordpromocode: { "1 Month": 0.6, "3 Months": 1.05, "Boost 1m": 4, "Boost 3m": 15 },
  ...
  "chatgpt-pro": { "1 Month": 3.15, "3 Months": 6 },
  capcut: { "1 Month": 1.2, "3 Months": 2.5, "6 Months": 4.2, "12 Months": 8, "Lifetime": 2.35 },
  geoguessr: { "1 Month": 2, "3 Months": 5, "12 Months": 10, "Lifetime": 0.81 },
  filmora: { "1 Month": 2.5, "3 Months": 6, "6 Months": 10.5, "12 Months": 16, "Lifetime": 7.29 },
  duolingo: { "12 Months": 1.24, "Lifetime": 1.62 },
  movistar: { "12 Months": 2.44, "Lifetime": 2.92 },
  dazn: { "Lifetime": 1.35 },
  steamaccount: { "Random Games": 0.25 },
  microsoft: { "Random Codes": 0.4 },
  rockstar: { "Activation Code": 0.25 },
  minecraft: { "NFA Lifetime": 1, "FA Lifetime": 4.5 },
  stake: { "Level 2 Verified": 0.6 },
  xbox: { "Game Pass Lifetime": 0.6 }
};
function normalizePlan2(raw) {
  if (!raw)
    return raw;
  const key = raw.trim();
  return key;
}
__name(normalizePlan2, "normalizePlan");
function validateAndPriceCart2(cart) {
  let totalUSD = 0;
  const normalized = [];
  for (const item of cart) {
    if (!item.pid || !item.plan)
      throw new Error("Missing pid or plan");
    const table3 = PRICES_USD[item.pid];
    if (!table3)
      throw new Error(`Unknown product: ${item.pid}`);
    const plan = normalizePlan2(item.plan);
    const unit = table3[plan];
    if (unit === void 0)
...
__name(validateAndPriceCart2, "validateAndPriceCart");
var crypto_now_create_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS")
    ...
    if (request.method !== "POST")
      return new Response("Method Not Allowed", { status: 405 });
    const url = new URL(request.url);
    if (!url.pathname.startsWith("/api/crypto/now/create"))
      return new Response("Not Found", { status: 404 });
    try {
      const { cart, customerEmail, payCurrency } = await request.json();
      if (!Array.isArray(cart) || !cart.length)
        return new Response(JSON.stringify({ error: "Empty cart" }), { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
      if (!customerEmail)
        return new Response(JSON.stringify({ error: "Missing customerEmail" }), { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
      const { totalUSD, items } = validateAndPriceCart2(cart);
      const apiKey = env2.NOWPAYMENTS_API_KEY;
      if (!apiKey)
      ...
      const body = {
        ...
        ipn_callback_url: (env2.APP_URL || "https://plugmarket.es") + "/api/crypto/now/ipn",
        order_id: orderId,
...
// functions/crypto-now-ipn.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase5(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_ANON_KEY;
  if (!url || !key)
    throw new Error("Missing Supabase credentials");
...
__name(getSupabase5, "getSupabase");
async function assignAccount2(env2, productId, plan, customerEmail) {
  const supabase = getSupabase5(env2);
  const { data: accounts, error: fetchError } = await supabase.from("accounts").select("*").eq("product_id", productId).eq("plan", plan).eq("status", "available").order("created_at", { ascending: true }).order("id", { ascending: true }).limit(1);
  if (fetchError)
    throw new Error(`DB fetch error: ${fetchError.message}`);
  if (!accounts || accounts.length === 0)
    throw new Error(`No available accounts for ${productId} - ${plan}`);
  const account = accounts[0];
  const { error: updateError } = await supabase.from("accounts").update({ status: "sold", sold_at: (/* @__PURE__ */ new Date()).toISOString(), customer_email: customerEmail }).eq("id", account.id);
  if (updateError)
    throw new Error(`DB update error: ${updateError.message}`);
...
__name(assignAccount2, "assignAccount");
...
__name(hmacSHA512, "hmacSHA512");
var crypto_now_ipn_default = {
  async fetch(request, env2) {
    if (request.method !== "POST")
      return new Response("Method Not Allowed", { status: 405 });
    const url = new URL(request.url);
    if (!url.pathname.startsWith("/api/crypto/now/ipn"))
      return new Response("Not Found", { status: 404 });
    const secret = env2.NOWPAYMENTS_IPN_SECRET;
    if (!secret)
    ...
    try {
      ...
      for (const item of items) {
        ...
        for (let i = 0; i < qty; i++) {
          try {
            const creds = await assignAccount2(env2, item.pid, item.plan, customerEmail);
            const itemCreds = { email: creds.email, password: creds.password };
            if (creds.chatgptPassword)
              itemCreds.chatgptPassword = creds.chatgptPassword;
            if (creds.chatgptCode)
              itemCreds.chatgptCode = creds.chatgptCode;
      ...
      const { error: orderError } = await supabase.from("orders").insert({ customer_email: customerEmail, payment_intent_id: String(paymentId || orderId), total_cents: totalCents, items: fulfilled });
      if (orderError) {
...
// functions/crypto-now-status.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var crypto_now_status_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS")
    ...
    if (request.method === "GET") {
      const url = new URL(request.url);
      if (!url.pathname.startsWith("/api/crypto/now/status"))
        return new Response("Not Found", { status: 404 });
    ...
    if (!paymentId)
      return new Response(JSON.stringify({ error: "Missing paymentId" }), { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
    const apiKey = env2.NOWPAYMENTS_API_KEY;
    if (!apiKey)
...
// functions/coinbase-create-charge.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var PRICES_USD2 = {
  ...
  "disney": { "1 Month": 1.1, "3 Months": 2.7, "6 Months": 4.8, "12 Months": 9.5, "Lifetime": 0.89 },
  "amazon-prime": { "1 Month": 1.8, "3 Months": 3.4, "6 Months": 5.8, "12 Months": 10.64, "Lifetime": 14 },
  "prime": { "1 Month": 1.8, "3 Months": 3.4, "6 Months": 5.8, "12 Months": 10.64, "Lifetime": 14 },
  "hbo-max": { "1 Month": 1.4, "3 Months": 3.1, "6 Months": 4.7, "12 Months": 9.8, "Lifetime": 13 },
  "hbomax": { "1 Month": 1.4, "3 Months": 3.1, "6 Months": 4.7, "12 Months": 9.8, "Lifetime": 13 },
  "nord-vpn": { "1 Month": 0.85, "3 Months": 2, "6 Months": 3.6, "12 Months": 6.9, "Lifetime": 1.46 },
  "nordvpn": { "1 Month": 0.85, "3 Months": 2, "6 Months": 3.6, "12 Months": 6.9, "Lifetime": 1.46 },
  "crunchyroll": { "1 Month": 0.9, "3 Months": 2.1, "6 Months": 3.8, "12 Months": 7.2, "Lifetime": 1.05 },
  "crunchy": { "1 Month": 0.9, "3 Months": 2.1, "6 Months": 3.8, "12 Months": 7.2, "Lifetime": 1.05 },
  "discord-nitro": { "Boost 1m": 4, "Boost 1 Year": 15.17, "Basic 1m": 1.35 },
  "nitro": { "Boost 1m": 4, "Boost 1 Year": 15.17, "Basic 1m": 1.35 },
  "discordpromocode": { "1 Month": 0.6, "3 Months": 1.05, "Boost 1m": 4, "Boost 3m": 15 },
  "realmembers": { "[500]": 2.25, "[1000]": 4.25, "[2000]": 7.88, "[3000]": 11.63, "[4000]": 12.24, "[5000]": 15.61 },
  "chatgpt-plus": { "1 Month": 3.2, "3 Months": 7.8, "6 Months": 11.88, "12 Months": 8.5 },
  "chatgpt": { "1 Month": 3.2, "3 Months": 7.8, "6 Months": 11.88, "12 Months": 8.5 },
  "chatgpt-pro": { "1 Month": 3.15, "3 Months": 6 },
  "capcut-pro": { "1 Month": 1.2, "3 Months": 2.5, "6 Months": 4.2, "12 Months": 8, "Lifetime": 2.35 },
  "capcut": { "1 Month": 1.2, "3 Months": 2.5, "6 Months": 4.2, "12 Months": 8, "Lifetime": 2.35 },
  "geoguessr": { "1 Month": 2, "3 Months": 5, "12 Months": 10, "Lifetime": 0.81 },
  "filmora": { "1 Month": 2.5, "3 Months": 6, "6 Months": 10.5, "12 Months": 16, "Lifetime": 7.29 },
  "duolingo": { "12 Months": 1.24, "Lifetime": 1.62 },
...
function normalizePlan3(p) {
  const lower = String(p || "").toLowerCase().trim();
  if (lower === "1 mes" || lower === "1 month")
    return "1 Month";
  if (lower === "3 meses" || lower === "3 months")
    return "3 Months";
  if (lower === "6 meses" || lower === "6 months")
    return "6 Months";
  if (lower === "12 meses" || lower === "12 months")
    return "12 Months";
  if (lower === "24 meses" || lower === "24 months")
    return "24 Months";
  if (lower === "lifetime")
    return "Lifetime";
  return p;
}
__name(normalizePlan3, "normalizePlan");
function validateAndPriceCart3(cart) {
  if (!Array.isArray(cart) || cart.length === 0)
    throw new Error("Cart is empty");
  let total = 0;
  for (const item of cart) {
    let { pid: pid2, plan, qty } = item;
    if (!pid2 || !plan || !qty)
      throw new Error("Invalid cart item");
    if (pid2 === "nordvpn")
      pid2 = "nord-vpn";
    const norm = normalizePlan3(plan);
    const price = PRICES_USD2[pid2]?.[norm];
    if (price == null)
      throw new Error(`Invalid product or plan: ${pid2} - ${norm}`);
    total += price * qty;
  }
  return total;
}
__name(validateAndPriceCart3, "validateAndPriceCart");
async function handleCoinbaseCreateCharge(request, env2) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "POST, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type"
  ...
  try {
    ...
    const totalUSD = validateAndPriceCart3(cart);
    const apiKey = env2.COINBASE_API_KEY;
    if (!apiKey)
      throw new Error("COINBASE_API_KEY not configured");
    const metadata = {
      customerEmail,
      cart: cart.map((i) => ({ pid: i.pid, plan: normalizePlan3(i.plan), qty: i.qty }))
    };
    const chargePayload = {
...
__name(handleCoinbaseCreateCharge, "handleCoinbaseCreateCharge");

// functions/coinbase-get-charge.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function handleCoinbaseGetCharge(request, env2) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type"
...
__name(handleCoinbaseGetCharge, "handleCoinbaseGetCharge");

// functions/coinbase-webhook.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
__name(normalizePlan4, "normalizePlan");
async function handleCoinbaseWebhook(request, env2) {
  if (request.method !== "POST")
    return new Response("Method not allowed", { status: 405 });
  try {
    const rawBody = await request.text();
    const signature = request.headers.get("X-CC-Webhook-Signature");
    const webhookSecret = env2.COINBASE_WEBHOOK_SECRET;
    if (!webhookSecret) {
      console.error("COINBASE_WEBHOOK_SECRET not configured");
      return new Response("Webhook secret not configured", { status: 500 });
    }
    const computedSignature = crypto_default.createHmac("sha256", webhookSecret).update(rawBody).digest("hex");
    ...
    if (event.type === "charge:confirmed") {
      ...
      const supabase = createClient(env2.SUPABASE_URL, env2.SUPABASE_KEY);
      const assignedItems = [];
      for (const item of cart) {
        const { pid: pid2, plan, qty } = item;
        const norm = normalizePlan4(plan);
        for (let i = 0; i < qty; i++) {
          const { data: accounts, error: fetchError } = await supabase.from("accounts").select("id, email, password, chatgpt_password, chatgpt_code").eq("product_id", pid2).eq("plan", norm).eq("status", "available").limit(1);
          if (fetchError || !accounts || accounts.length === 0) {
            console.error(`No stock for ${pid2} - ${norm}`);
            break;
          }
          const account = accounts[0];
          const { error: updateError } = await supabase.from("accounts").update({ status: "sold" }).eq("id", account.id);
          if (updateError) {
          ...
          if (account.chatgpt_password)
            itemData.chatgptPassword = account.chatgpt_password;
          if (account.chatgpt_code)
            itemData.chatgptCode = account.chatgpt_code;
...
__name(handleCoinbaseWebhook, "handleCoinbaseWebhook");

// functions/send-verification-code.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var send_verification_code_default = {
  async fetch(request, env2, ctx) {
    if (request.method !== "POST") {
      return new Response(JSON.stringify({ error: "Method not allowed" }), {
        status: 405,
    ...
    try {
      ...
      await fetch(`${env2.SUPABASE_URL}/rest/v1/verification_codes?email=eq.${email}`, {
        method: "DELETE",
        headers: {
      ...
      await fetch(`${env2.SUPABASE_URL}/rest/v1/verification_codes`, {
        method: "POST",
        headers: {
          "apikey": env2.SUPABASE_ANON_KEY,
          "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`,
      ...
      const emailSent = await sendVerificationEmail(env2, email, code);
      if (!emailSent) {
      ...
      console.error("Error sending verification code:", error3);
...
async function sendVerificationEmail(env2, email, code) {
  try {
    const emailData = {
      ...
      html: `
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>\xA9 2025 Plug Market. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
...
var verify_code_default = {
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      const response = await fetch(
        `${env2.SUPABASE_URL}/rest/v1/verification_codes?email=eq.${email}&code=eq.${code}&expires_at=gt.${(/* @__PURE__ */ new Date()).toISOString()}&order=created_at.desc&limit=1`,
        {
          headers: {
            "apikey": env2.SUPABASE_ANON_KEY,
            "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`
      ...
      await fetch(`${env2.SUPABASE_URL}/rest/v1/verification_codes?email=eq.${email}&code=eq.${code}`, {
        method: "DELETE",
        headers: {
          "apikey": env2.SUPABASE_ANON_KEY,
          "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`
      ...
      await fetch(`${env2.SUPABASE_URL}/rest/v1/users`, {
        method: "POST",
        headers: {
          "apikey": env2.SUPABASE_ANON_KEY,
          "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`,
      ...
      const token = await generateToken(email, env2.JWT_SECRET);
      return new Response(JSON.stringify({
        success: true,
        token,
        email
      ...
      console.error("Error verifying code:", error3);
      return new Response(JSON.stringify({ error: "Internal server error" }), {
        status: 500,
        headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
      });
...
// functions/get-user-orders.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var get_user_orders_default = {
  async fetch(request, env2, ctx) {
    if (request.method !== "POST") {
      return new Response(JSON.stringify({ error: "Method not allowed" }), {
        status: 405,
    ...
    try {
      ...
      const isValid = await verifyToken(token, env2.JWT_SECRET);
      if (!isValid) {
      ...
      const userResponse = await fetch(
        `${env2.SUPABASE_URL}/rest/v1/users?email=eq.${email}`,
        {
          headers: {
            "apikey": env2.SUPABASE_ANON_KEY,
            "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`
      ...
      console.log(`\u{1F50D} Querying orders for email: ${email}`);
      const ordersResponse = await fetch(
        `${env2.SUPABASE_URL}/rest/v1/orders?customer_email=eq.${email}&order=created_at.desc`,
        {
          headers: {
            "apikey": env2.SUPABASE_ANON_KEY,
            "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`
      ...
      console.log(`\u{1F4E6} Found ${orders?.length || 0} orders for ${email}`);
      console.log(`\u{1F4CB} Orders data:`, JSON.stringify(orders).slice(0, 500));
      return new Response(JSON.stringify({
        success: true,
      ...
      console.error("Error getting user orders:", error3);
      return new Response(JSON.stringify({ error: "Internal server error" }), {
        status: 500,
        headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
      });
...
var delete_account_default = {
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      const token = authHeader.substring(7);
      const isValid = await verifyToken2(token, env2.JWT_SECRET);
      if (!isValid) {
        return new Response(JSON.stringify({ error: "Invalid token" }), {
          status: 401,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
      ...
      await fetch(`${env2.SUPABASE_URL}/rest/v1/users?email=eq.${email}`, {
        method: "DELETE",
        headers: {
          "apikey": env2.SUPABASE_ANON_KEY,
          "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`
      ...
      await fetch(`${env2.SUPABASE_URL}/rest/v1/orders?customer_email=eq.${email}`, {
        method: "PATCH",
        headers: {
          "apikey": env2.SUPABASE_ANON_KEY,
          "Authorization": `Bearer ${env2.SUPABASE_ANON_KEY}`,
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function handler(request, env2) {
  if (request.method !== "GET") {
    return new Response(JSON.stringify({ error: "Method not allowed" }), {
      status: 405,
      headers: {
  ...
  try {
    ...
    const supabaseUrl = env2.SUPABASE_URL;
    const supabaseKey = env2.SUPABASE_SERVICE_ROLE_KEY || env2.SUPABASE_ANON_KEY;
    ...
    console.error("Error getting balance:", error3);
    return new Response(JSON.stringify({
...
// functions/create-topup-intent.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function handler2(request, env2) {
  if (request.method !== "POST") {
    return new Response(JSON.stringify({ error: "Method not allowed" }), {
      status: 405,
      headers: {
  ...
  try {
    ...
    const stripeSecretKey = env2.STRIPE_SECRET_KEY;
    ...
    if (!paymentIntentResponse.ok) {
      const error3 = await paymentIntentResponse.text();
      throw new Error(`Stripe error: ${error3}`);
    }
    ...
    console.error("Error creating topup intent:", error3);
    return new Response(JSON.stringify({
...
// functions/get-balance-transactions.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function handler3(request, env2) {
  if (request.method !== "GET") {
    return new Response(JSON.stringify({ error: "Method not allowed" }), {
      status: 405,
      headers: {
  ...
  try {
    ...
    const supabaseUrl = env2.SUPABASE_URL;
    const supabaseKey = env2.SUPABASE_SERVICE_ROLE_KEY;
    const response = await fetch(
    ...
    console.error("Error getting transactions:", error3);
    return new Response(JSON.stringify({
...
// functions/pay-with-balance.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase6(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_SERVICE_ROLE_KEY || env2.SUPABASE_ANON_KEY;
  if (!url || !key)
    throw new Error("Missing Supabase credentials");
  return createClient(url, key);
}
__name(getSupabase6, "getSupabase");
async function assignAccount3(env2, productId, plan, customerEmail) {
  const supabase = getSupabase6(env2);
  const { data: accounts, error: error3 } = await supabase.from("accounts").select("*").eq("product_id", productId).eq("plan", plan).eq("status", "available").order("created_at", { ascending: true }).limit(1);
  if (error3)
    throw new Error(`DB fetch error: ${error3.message}`);
  if (!accounts || accounts.length === 0)
    throw new Error(`No available accounts for ${productId} - ${plan}`);
  const account = accounts[0];
  const { error: updateError } = await supabase.from("accounts").update({ status: "sold", sold_at: (/* @__PURE__ */ new Date()).toISOString(), customer_email: customerEmail }).eq("id", account.id);
...
__name(assignAccount3, "assignAccount");
async function handler4(request, env2) {
  if (request.method !== "POST") {
    return new Response(JSON.stringify({ error: "Method not allowed" }), {
      status: 405,
      headers: {
  ...
  try {
    ...
    for (const item of cart) {
      ...
      for (let i = 0; i < qty; i++) {
        try {
          const credentials = await assignAccount3(env2, item.pid, item.plan, email);
          ...
          if (credentials.chatgptPassword)
            itemCreds.credentials.chatgptPassword = credentials.chatgptPassword;
          if (credentials.chatgptCode)
            itemCreds.credentials.chatgptCode = credentials.chatgptCode;
    ...
    const supabase = getSupabase6(env2);
    const { data: orders, error: orderError } = await supabase.from("orders").insert(orderData).select();
    if (orderError) {
    ...
    const { error: transactionError } = await supabase.from("balance_transactions").insert(transactionData);
    if (transactionError) {
    ...
    console.error("Error processing balance payment:", error3);
...
// functions/paypal-ff-create-order.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var PRICES_EUR2 = {
  boosts: { "3 Months": 6.65 },
  netflix: { "1 Month": 1.22, "3 Months": 2.84, "6 Months": 4.86, "12 Months": 8.91, "Bulk": 0.49, "Lifetime": 1.1, "FA": 8.5 },
  spotify: { "1 Month": 1.78, "3 Months": 3.08, "6 Months": 5.02, "12 Months": 9.62, "Lifetime": 1.78 },
  "youtube-premium": { "1 Month": 2.27, "6 Months": 6.8, "12 Months": 10.13, "Lifetime": 2.27, "fowner": 4.74 },
  disney: { "1 Month": 0.89, "3 Months": 2.19, "6 Months": 3.89, "12 Months": 7.7, "Lifetime": 0.89 },
  prime: { "1 Month": 0.65, "3 Months": 1.78, "12 Months": 5.99, "Lifetime": 0.65 },
  hbomax: { "Lifetime": 1.08 },
  nordvpn: { "1 Month": 1.46, "3 Months": 2.75, "6 Months": 4.7, "12 Months": 8.62, "Lifetime": 1.46 },
  crunchy: { "1 Month": 1.05, "3 Months": 2.67, "6 Months": 4.7, "12 Months": 8.42, "Lifetime": 1.05 },
  nitro: { "Boost 1m": 4, "Boost 1 Year": 12.29, "Basic 1m": 1.1 },
  discordpromocode: { "1 Month": 0.62, "3 Months": 1.19, "Boost 1m": 4, "Boost 3m": 13.65 },
  ...
  "chatgpt-pro": { "1 Month": 2.84, "3 Months": 5.4 },
  capcut: { "1 Month": 1.08, "3 Months": 2.25, "6 Months": 3.78, "12 Months": 7.2, "Lifetime": 2.12 },
  geoguessr: { "1 Month": 1.8, "3 Months": 4.5, "12 Months": 9, "Lifetime": 0.73 },
  filmora: { "1 Month": 2.25, "3 Months": 5.4, "6 Months": 9.45, "12 Months": 14.4, "Lifetime": 6.56 },
  duolingo: { "12 Months": 1.12, "Lifetime": 1.46 },
  movistar: { "12 Months": 2.2, "Lifetime": 2.63 },
  dazn: { "Lifetime": 1.22 },
  steamaccount: { "Random Games": 0.25 },
  microsoft: { "Random Codes": 0.4 },
  rockstar: { "Activation Code": 0.25 },
...
__name(getRandomNote, "getRandomNote");
...
__name(getVolumeDiscount, "getVolumeDiscount");
var paypal_ff_create_order_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
    ...
    try {
      ...
      for (const item of cart) {
        const { pid: pid2, plan, qty } = item;
        if (!pid2 || !plan || !qty || qty < 1) {
          return new Response(JSON.stringify({ error: "Invalid item in cart" }), {
            status: 400,
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
          });
        }
        const productPrices = PRICES_EUR2[pid2];
        if (!productPrices || !productPrices[plan]) {
          return new Response(JSON.stringify({ error: `Invalid product or plan: ${pid2} - ${plan}` }), {
            status: 400,
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
          });
        }
        ...
        subtotal += price * qty;
        totalQty += qty;
        items.push({ pid: pid2, plan, qty, price });
      }
      ...
      return new Response(JSON.stringify({
        orderId,
        email: env2.PAYPAL_MANUAL_EMAIL || "payments@plugmarket.es",
        amount: parseFloat(total.toFixed(2)),
        note,
        instructions: {
          step1: "Open PayPal app or website",
          step2: `Send $${total.toFixed(2)} to ${env2.PAYPAL_MANUAL_EMAIL || "payments@plugmarket.es"}`,
...
// functions/paypal-ff-webhook.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function assignAccount4(supabaseUrl, supabaseKey, productId, plan, customerEmail) {
...
__name(assignAccount4, "assignAccount");
var paypal_ff_webhook_default = {
  async fetch(request, env2) {
    if (request.method !== "POST") {
      return new Response("Method Not Allowed", { status: 405 });
    }
    try {
      ...
      const verified = await verifyIPN(body, env2);
      if (!verified) {
      ...
      const expectedEmail = env2.PAYPAL_MANUAL_EMAIL;
      ...
      try {
        for (const item of cartItems) {
          ...
          for (let i = 0; i < qty; i++) {
            const credentials = await assignAccount4(
              supabaseUrl,
              supabaseKey,
              item.pid,
              item.plan,
...
async function verifyIPN(body, env2) {
  try {
    const urls = [
...
__name(verifyIPN, "verifyIPN");

// functions/paypal-ff-check-order.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var paypal_ff_check_order_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, { headers: {
        "Access-Control-Allow-Origin": "*",
    ...
    try {
      ...
      if (orders.length === 0) {
        response = await fetch(
          `${supabaseUrl}/rest/v1/orders?payment_intent_id=ilike.${orderId}_completed_%`,
          {
            headers: {
              "apikey": supabaseKey,
              "Authorization": `Bearer ${supabaseKey}`
...
// functions/paypal-ff-manual-complete.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function assignAccount5(supabaseUrl, supabaseKey, productId, plan, customerEmail) {
  console.log(`Assigning account: product_id="${productId}", plan="${plan}"`);
  const fetchRes = await fetch(
    `${supabaseUrl}/rest/v1/accounts?product_id=eq.${productId}&plan=eq.${plan}&status=eq.available&order=created_at.asc,id.asc&limit=1`,
    {
...
__name(assignAccount5, "assignAccount");
var paypal_ff_manual_complete_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, { headers: {
        "Access-Control-Allow-Origin": "*",
    ...
    try {
      ...
      for (const item of cartItems) {
        ...
        for (let i = 0; i < qty; i++) {
          try {
            const credentials = await assignAccount5(
              supabaseUrl,
              supabaseKey,
              item.pid,
              item.plan,
...
// functions/search-orders.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase7(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_ANON_KEY;
  if (!url || !key) {
    throw new Error("Missing Supabase credentials");
...
__name(getSupabase7, "getSupabase");
var search_orders_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, {
        headers: {
    ...
    const email = url.searchParams.get("email");
    try {
      const supabase = getSupabase7(env2);
      const sinceDate = /* @__PURE__ */ new Date();
      sinceDate.setDate(sinceDate.getDate() - days);
      let query = supabase.from("orders").select("*").gte("created_at", sinceDate.toISOString()).order("created_at", { ascending: false });
      const { data: orders, error: error3 } = await query;
      if (error3) {
        console.error("Supabase error:", error3);
        return new Response(
      ...
      if (product) {
        filteredOrders = filteredOrders.filter((order) => {
          ...
          return items.some(
            (item) => item.pid && item.pid.toLowerCase().includes(product.toLowerCase())
          );
        });
      }
      if (email) {
        ...
        filteredOrders = filteredOrders.filter(
          (order) => order.customer_email && order.customer_email.toLowerCase().trim() === normalizedEmail
        );
      }
      ...
      console.error("Error searching orders:", error3);
      return new Response(
        JSON.stringify({ error: error3.message }),
        {
          status: 500,
...
// functions/find-missing-orders.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase8(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_ANON_KEY;
  if (!url || !key) {
    throw new Error("Missing Supabase credentials");
...
__name(getSupabase8, "getSupabase");
var find_missing_orders_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, {
        headers: {
    ...
    try {
      const supabase = getSupabase8(env2);
      const normalizedEmail = email.toLowerCase().trim();
      const sinceDate = /* @__PURE__ */ new Date();
      sinceDate.setDate(sinceDate.getDate() - days);
      console.log(`\u{1F50D} Searching for missing orders for: ${normalizedEmail}`);
      const { data: orders } = await supabase.from("orders").select("id, customer_email, total_cents, payment_intent_id, created_at, items").eq("customer_email", normalizedEmail).gte("created_at", sinceDate.toISOString()).order("created_at", { ascending: false });
      console.log(`\u{1F4E6} Found ${orders?.length || 0} orders`);
      const { data: transactions } = await supabase.from("balance_transactions").select("*").eq("user_email", normalizedEmail).gte("created_at", sinceDate.toISOString()).order("created_at", { ascending: false });
      console.log(`\u{1F4B0} Found ${transactions?.length || 0} balance transactions`);
      ...
      console.error("Error finding missing orders:", error3);
      return new Response(
        JSON.stringify({
          error: error3.message,
          stack: error3.stack
...
// functions/recover-stripe-order.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase9(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_SERVICE_ROLE_KEY || env2.SUPABASE_ANON_KEY;
  if (!url || !key)
    throw new Error("Missing Supabase credentials");
...
__name(getSupabase9, "getSupabase");
async function assignAccount6(env2, productId, plan, customerEmail) {
  const supabase = getSupabase9(env2);
  const { data: accounts, error: error3 } = await supabase.from("accounts").select("*").eq("product_id", productId).eq("plan", plan).eq("status", "available").order("created_at", { ascending: true }).limit(1);
  if (error3)
    throw new Error(`DB fetch error: ${error3.message}`);
  if (!accounts || accounts.length === 0)
...
__name(assignAccount6, "assignAccount");
var recover_stripe_order_default = {
  async fetch(request, env2) {
    if (request.method !== "POST") {
      return new Response(JSON.stringify({ error: "Method not allowed" }), {
        status: 405,
    ...
    try {
      ...
      console.log(`\u{1F527} Recovering order for payment intent: ${payment_intent_id}`);
      const supabase = getSupabase9(env2);
      const { data: existingOrders } = await supabase.from("orders").select("id").eq("payment_intent_id", payment_intent_id);
      ...
      console.log(`\u{1F511} Fetching payment intent from Stripe...`);
      ...
      console.log(`\u{1F4E1} Stripe response status: ${piResponse.status}`);
      if (!piResponse.ok) {
      ...
      console.log(`\u{1F4E7} Customer: ${customerEmail}`);
      console.log(`\u{1F6D2} Cart:`, cart);
      const orderItems = [];
      for (const item of cart) {
        ...
        for (let i = 0; i < qty; i++) {
          try {
            const credentials = await assignAccount6(env2, item.pid, item.plan, customerEmail);
            orderItems.push({
              pid: item.pid,
              name: item.pid,
              plan: item.plan,
      ...
      console.error("Error recovering order:", error3);
      return new Response(JSON.stringify({
        error: error3.message,
        stack: error3.stack
      }), {
...
// functions/manual-create-order.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getSupabase10(env2) {
  const url = env2.SUPABASE_URL;
  const key = env2.SUPABASE_SERVICE_ROLE_KEY || env2.SUPABASE_ANON_KEY;
  if (!url || !key)
    throw new Error("Missing Supabase credentials");
...
__name(getSupabase10, "getSupabase");
async function assignAccounts(env2, productId, plan, quantity, customerEmail) {
  const supabase = getSupabase10(env2);
  const { data: accounts, error: error3 } = await supabase.from("accounts").select("*").eq("product_id", productId).eq("plan", plan).eq("status", "available").order("created_at", { ascending: true }).limit(quantity);
  if (error3)
    throw new Error(`DB fetch error: ${error3.message}`);
  if (!accounts || accounts.length === 0)
    throw new Error(`No available accounts for ${productId} - ${plan}`);
  const accountIds = accounts.map((a) => a.id);
  const { error: updateError } = await supabase.from("accounts").update({ status: "sold", sold_at: (/* @__PURE__ */ new Date()).toISOString(), customer_email: customerEmail }).in("id", accountIds);
  if (updateError)
    throw new Error(`DB update error: ${updateError.message}`);
...
__name(assignAccounts, "assignAccounts");
var manual_create_order_default = {
  async fetch(request, env2) {
    if (request.method !== "POST") {
    ...
    try {
      ...
      for (const item of items) {
        const qty = Number(item.qty) || 1;
        const credentials = await assignAccounts(env2, item.pid, item.plan, qty, customerEmail);
      ...
      const { data: orders, error: orderError } = await supabase.from("orders").insert({ customer_email: customerEmail, payment_intent_id: payment_reference, total_cents, items: orderItems }).select();
      if (orderError)
        throw new Error(`Failed to create order: ${orderError.message}`);
      return new Response(JSON.stringify({ success: true, order_id: orders[0].id, items_assigned: orderItems.length, customer_email: customerEmail, total_cents }), { status: 200, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
    } catch (error3) {
      return new Response(JSON.stringify({ error: error3.message }), { status: 500, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
    }
  }
};
...
var worker_default = {
  async fetch(request, env2, ctx) {
    ...
    if (path === "/api/get-stripe-config")
      return get_stripe_config_default.fetch(request, env2, ctx);
    if (path === "/api/create-payment-intent")
      return create_payment_intent_default.fetch(request, env2, ctx);
    if (path === "/api/stripe-webhook")
      return stripe_webhook_default.fetch(request, env2, ctx);
    if (path === "/api/recover-stripe-order")
      return recover_stripe_order_default.fetch(request, env2, ctx);
    if (path === "/api/manual-create-order")
      return manual_create_order_default.fetch(request, env2, ctx);
    if (path === "/api/get-orders")
      return get_orders_default.fetch(request, env2, ctx);
    if (path === "/api/search-orders")
      return search_orders_default.fetch(request, env2, ctx);
    if (path === "/api/find-missing-orders")
      return find_missing_orders_default.fetch(request, env2, ctx);
    if (path === "/api/get-stock")
      return get_stock_default.fetch(request, env2, ctx);
    if (path === "/api/debug-netflix-accounts")
      return debug_netflix_accounts_default.fetch(request, env2, ctx);
    if (path === "/api/paypal-ff/create-order")
      return paypal_ff_create_order_default.fetch(request, env2, ctx);
    if (path === "/api/paypal-ff/webhook")
      return paypal_ff_webhook_default.fetch(request, env2, ctx);
    if (path === "/api/paypal-ff/check-order")
      return paypal_ff_check_order_default.fetch(request, env2, ctx);
    if (path === "/api/paypal-ff/manual-complete")
      return paypal_ff_manual_complete_default.fetch(request, env2, ctx);
    if (path === "/api/crypto/now/create")
      return crypto_now_create_default.fetch(request, env2, ctx);
    if (path === "/api/crypto/now/ipn")
      return crypto_now_ipn_default.fetch(request, env2, ctx);
    if (path === "/api/crypto/now/status")
      return crypto_now_status_default.fetch(request, env2, ctx);
    if (path === "/api/coinbase/create-charge")
      return handleCoinbaseCreateCharge(request, env2);
    if (path === "/api/coinbase/get-charge")
      return handleCoinbaseGetCharge(request, env2);
    if (path === "/api/coinbase/webhook")
      return handleCoinbaseWebhook(request, env2);
    if (path === "/api/submit-review")
      return submit_review_default.fetch(request, env2, ctx);
    if (path === "/api/get-reviews")
      return get_reviews_default.fetch(request, env2, ctx);
    if (path === "/api/send-verification-code")
      return send_verification_code_default.fetch(request, env2, ctx);
    if (path === "/api/verify-code")
      return verify_code_default.fetch(request, env2, ctx);
    if (path === "/api/get-user-orders")
      return get_user_orders_default.fetch(request, env2, ctx);
    if (path === "/api/delete-account")
      return delete_account_default.fetch(request, env2, ctx);
    if (path === "/api/get-balance")
      return handler(request, env2);
    if (path === "/api/create-topup-intent")
      return handler2(request, env2);
    if (path === "/api/get-balance-transactions")
      return handler3(request, env2);
    if (path === "/api/pay-with-balance")
      return handler4(request, env2);
    return new Response("Not Found", { status: 404 });
  }
};
export {
