...
// .wrangler/tmp/bundle-cAlm59/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-cAlm59/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-cAlm59/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var PRICES_USD = {
  ...
  geoguessr: { "1 Month": 200, "3 Months": 500, "12 Months": 1e3 },
...
var fetchWithAuth = /* @__PURE__ */ __name((supabaseKey, getAccessToken3, customFetch) => {
  const fetch2 = resolveFetch4(customFetch);
  const HeadersConstructor = resolveHeadersConstructor();
  return async (input, init) => {
    var _a2;
    const accessToken = (_a2 = await getAccessToken3()) !== null && _a2 !== void 0 ? _a2 : supabaseKey;
...
var get_orders_default = {
  async fetch(request, env2) {
    ...
    try {
      ...
      const { data: orders, error: error3 } = await supabase.from("orders").select("*").eq("customer_email", email.toLowerCase().trim()).order("created_at", { ascending: false });
      if (error3) {
...
function normalizePlan2(raw) {
  ...
  return PLAN_ALIASES2[key] || raw.trim();
}
...
function encodeBasicAuth(id, secret) {
  ...
  try {
    ...
    return Buffer2.from(pair).toString("base64");
  }
}
...
function validateAndPriceCart2(cart) {
  ...
  for (const item of cart) {
    ...
    const priceTable = PRICES_USD2[item.pid];
    if (!priceTable)
      throw new Error(`Unknown product: ${item.pid}`);
    const originalPlan = item.plan;
    const plan = normalizePlan2(originalPlan);
...
async function getAccessToken(env2) {
  const base = (env2.PAYPAL_ENV || "sandbox") === "live" ? "https://api-m.paypal.com" : "https://api-m.sandbox.paypal.com";
  const clientId = env2.PAYPAL_CLIENT_ID;
  const secret = env2.PAYPAL_SECRET;
  if (!clientId || !secret)
  ...
  if (!res.ok)
    throw new Error(json.error_description || json.error || "Failed to get PayPal token");
  return { token: json.access_token, base };
}
__name(getAccessToken, "getAccessToken");
var paypal_create_order_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS")
    ...
    if (request.method !== "POST")
      return new Response("Method Not Allowed", { status: 405 });
    const url = new URL(request.url);
    if (!url.pathname.startsWith("/api/paypal/create-order"))
      return new Response("Not Found", { status: 404 });
    try {
      const { cart } = await request.json();
      ...
      const totalCents = validateAndPriceCart2(cart);
      let amount = (totalCents / 100).toFixed(2);
      const currency = env2.PAYPAL_CURRENCY || "USD";
      const reference = buildReference();
      const { token, base } = await getAccessToken(env2);
      if ((env2.PAYPAL_ENV || "sandbox") === "live") {
...
// functions/paypal-capture-order.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
async function getAccessToken2(env2) {
  ...
  const creds = typeof btoa === "function" ? btoa(pair) : Buffer3.from(pair).toString("base64");
  const res = await fetch(base + "/v1/oauth2/token", {
    method: "POST",
    headers: { "Authorization": `Basic ${creds}`, "Content-Type": "application/x-www-form-urlencoded" },
    body: "grant_type=client_credentials"
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(json.error_description || "Failed to get token");
  return { token: json.access_token, base };
}
__name(getAccessToken2, "getAccessToken");
var paypal_capture_order_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS")
      return new Response(null, { headers: { "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "POST, OPTIONS", "Access-Control-Allow-Headers": "Content-Type" } });
    if (request.method !== "POST")
      return new Response("Method Not Allowed", { status: 405 });
    const url = new URL(request.url);
    if (!url.pathname.startsWith("/api/paypal/capture-order"))
      return new Response("Not Found", { status: 404 });
    try {
      ...
      if (!cart || !Array.isArray(cart) || !cart.length)
        return new Response(JSON.stringify({ error: "Missing cart" }), { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
      if (!customerEmail)
      ...
      const { token, base } = await getAccessToken2(env2);
      ...
      const data = await res.json();
      if (!res.ok)
        return new Response(JSON.stringify({ error: data.message || "Capture failed" }), { status: res.status, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
      ...
      for (const item of cart) {
        ...
        for (let i = 0; i < qty; i++) {
          try {
            const credentials = await assignAccount2(env2, item.pid, item.plan, customerEmail);
...
init_performance2();
var PRICES_USD3 = {
  netflix: { "1 Month": 1.5, "3 Months": 3.5, "6 Months": 6, "12 Months": 11, "Lifetime": 16.65 },
  ...
  nordvpn: { "1 Month": 0.85, "3 Months": 2, "6 Months": 3.6, "12 Months": 6.9 },
  ...
  "chatgpt-pro": { "1 Month": 3.5, "3 Months": 6 },
  capcut: { "1 Month": 1.2, "3 Months": 2.5, "6 Months": 4.2, "12 Months": 8 },
  geoguessr: { "1 Month": 2, "3 Months": 5, "12 Months": 10 },
  filmora: { "1 Month": 2.5, "3 Months": 6, "6 Months": 10.5, "12 Months": 16 },
  duolingo: { "12 Months": 1.24 },
  movistar: { "12 Months": 2.44 },
  dazn: { "Lifetime": 1.5 },
  steamaccount: { "Random Games": 0.25 }
};
function normalizePlan3(raw) {
  if (!raw)
    return raw;
  const key = raw.trim();
  return key;
...
function validateAndPriceCart3(cart) {
  ...
  for (const item of cart) {
    if (!item.pid || !item.plan)
      throw new Error("Missing pid or plan");
    const table3 = PRICES_USD3[item.pid];
    if (!table3)
      throw new Error(`Unknown product: ${item.pid}`);
    const plan = normalizePlan3(item.plan);
...
var crypto_now_create_default = {
  async fetch(request, env2) {
    ...
    try {
      ...
      if (!customerEmail)
        return new Response(JSON.stringify({ error: "Missing customerEmail" }), { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
      const { totalUSD, items } = validateAndPriceCart3(cart);
      ...
      const body = {
        ...
        ipn_callback_url: (env2.APP_URL || "https://plugmarket-api.laambedoor.workers.dev") + "/api/crypto/now/ipn",
        order_id: orderId,
  ...
  async fetch(request, env2) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        for (let i = 0; i < qty; i++) {
          try {
            const creds = await assignAccount3(env2, item.pid, item.plan, customerEmail);
...
var PRICES_USD4 = {
  ...
  "nord-vpn": { "1 Month": 0.85, "3 Months": 2, "6 Months": 3.6, "12 Months": 6.9 },
  ...
  "chatgpt-pro": { "1 Month": 3.5, "3 Months": 6 },
  "capcut-pro": { "1 Month": 1.2, "3 Months": 2.5, "6 Months": 4.2, "12 Months": 8 },
  "geoguessr": { "1 Month": 2, "3 Months": 5, "12 Months": 10 },
  "filmora": { "1 Month": 2.5, "3 Months": 6, "6 Months": 10.5, "12 Months": 16 },
  "duolingo": { "12 Months": 1.24 },
  "movistar": { "12 Months": 2.44 },
...
function validateAndPriceCart4(cart) {
  if (!Array.isArray(cart) || cart.length === 0)
  ...
  for (const item of cart) {
    ...
    const price = PRICES_USD4[pid2]?.[norm];
    if (price == null)
...
__name(validateAndPriceCart4, "validateAndPriceCart");
async function handleCoinbaseCreateCharge(request, env2) {
  ...
  try {
    ...
    const totalUSD = validateAndPriceCart4(cart);
    const apiKey = env2.COINBASE_API_KEY;
    if (!apiKey)
      throw new Error("COINBASE_API_KEY not configured");
    const metadata = {
      customerEmail,
      cart: cart.map((i) => ({ pid: i.pid, plan: normalizePlan4(i.plan), qty: i.qty }))
    };
    const chargePayload = {
...
__name(normalizePlan5, "normalizePlan");
  ...
  try {
    ...
    if (event.type === "charge:confirmed") {
      ...
      for (const item of cart) {
        ...
        const norm = normalizePlan5(plan);
        for (let i = 0; i < qty; i++) {
...
async function assignAccount4(env2, productId, plan, customerEmail) {
...
async function handler4(request, env2) {
  ...
  try {
    ...
    for (const item of cart) {
      ...
      for (let i = 0; i < qty; i++) {
        try {
          const credentials = await assignAccount4(env2, item.pid, item.plan, email);
...
function normalizePlan6(raw) {
  if (!raw)
    return raw;
  const key = raw.trim().toLowerCase();
  return PLAN_ALIASES3[key] || raw.trim();
}
__name(normalizePlan6, "normalizePlan");
var CASUAL_NOTES = [
  "Gas station",
  "Coffee",
  "Lunch",
...
var paypal_ff_create_order_default = {
  async fetch(request, env2) {
    ...
    try {
      ...
      for (const item of cart) {
        const { pid: pid2, plan, qty } = item;
        const normalizedPlan = normalizePlan6(plan);
        const priceMap = PRICES_USD5[pid2];
        if (!priceMap) {
          return new Response(JSON.stringify({ error: `Unknown product: ${pid2}` }), {
            status: 400,
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
          });
        }
        ...
        if (priceCents === void 0) {
          return new Response(JSON.stringify({ error: `Unknown plan: ${normalizedPlan} for ${pid2}` }), {
            status: 400,
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
          });
        }
      ...
      const amountEUR = (amountUSD * 0.92).toFixed(2);
      const paypalEmail = env2.PAYPAL_MANUAL_EMAIL || "soyalexesp123@gmail.com";
      const supabaseUrl = env2.SUPABASE_URL;
      const supabaseKey = env2.SUPABASE_ANON_KEY;
...
var paypal_ff_webhook_default = {
  async fetch(request, env2) {
    ...
    try {
      ...
      const verificationResponse = await verifyIPN(ipnData, env2);
      ...
      const supabase = createClient(
        env2.SUPABASE_URL,
        env2.SUPABASE_ANON_KEY
      );
      const { data: order, error: findError } = await supabase.from("orders").select("*").eq("payment_intent_id", casualNote).single();
      if (findError || !order) {
        console.error("Order not found for note:", casualNote);
        console.error("Find error:", findError);
        const { data: allOrders } = await supabase.from("orders").select("id, payment_intent_id, customer_email, total_cents").order("created_at", { ascending: false }).limit(10);
...
async function verifyIPN(ipnData, env2) {
  try {
    const isProduction = env2.PAYPAL_ENV === "live" || env2.PAYPAL_ENV === "production";
    const verifyUrl = isProduction ? "https://ipnpb.paypal.com/cgi-bin/webscr" : "https://ipnpb.sandbox.paypal.com/cgi-bin/webscr";
...
// functions/paypal-ff-test-complete.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var paypal_ff_test_complete_default = {
  async fetch(request, env2) {
    if (request.method === "OPTIONS") {
      return new Response(null, {
        headers: {
    ...
    try {
      ...
      const supabase = createClient(env2.SUPABASE_URL, env2.SUPABASE_ANON_KEY);
      const { data: order, error: findError } = await supabase.from("orders").select("*").eq("payment_intent_id", casualNote).single();
      if (findError || !order) {
        return new Response(JSON.stringify({
      ...
      const { error: updateError } = await supabase.from("orders").update({ payment_intent_id: testTxnId }).eq("id", order.id);
      if (updateError) {
...
// functions/paypal-ff-auto-verify.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function getPayPalAccessToken(env2) {
  const auth = btoa(`${env2.PAYPAL_CLIENT_ID}:${env2.PAYPAL_SECRET}`);
...
async function getRecentTransactions(accessToken, env2) {
  const endDate = /* @__PURE__ */ new Date();
  const startDate = new Date(endDate.getTime() - 10 * 60 * 1e3);
...
__name(getRecentTransactions, "getRecentTransactions");
async function processTransactions(transactions, env2) {
  const supabase = createClient(env2.SUPABASE_URL, env2.SUPABASE_ANON_KEY);
  ...
  for (const transaction of transactions) {
    if (transaction.transaction_info?.transaction_status !== "S")
      continue;
    if (transaction.transaction_info?.transaction_event_code !== "T0006")
      continue;
    ...
    const note = transaction.transaction_info?.transaction_note || transaction.transaction_info?.transaction_subject || "";
    ...
    if (!note)
      continue;
    const { data: orders, error: error3 } = await supabase.from("orders").select("*").eq("payment_intent_id", note).lt("payment_intent_id", "aaa").limit(1);
    if (error3) {
      console.error("[AUTO-VERIFY] Database error:", error3);
      continue;
    }
    ...
    const { error: updateError } = await supabase.from("orders").update({ payment_intent_id: txnId }).eq("id", order.id);
    if (updateError) {
...
__name(processTransactions, "processTransactions");
var paypal_ff_auto_verify_default = {
  async fetch(request, env2, ctx) {
    console.log("[AUTO-VERIFY] Starting automatic verification...");
    try {
      const accessToken = await getPayPalAccessToken(env2);
      console.log("[AUTO-VERIFY] Got PayPal access token");
      const transactions = await getRecentTransactions(accessToken, env2);
      const completedCount = await processTransactions(transactions, env2);
      ...
      console.error("[AUTO-VERIFY] Error:", error3);
      return new Response(JSON.stringify({
        success: false,
        error: error3.message
      }), {
  ...
  async scheduled(event, env2, ctx) {
    console.log("[AUTO-VERIFY] Cron trigger fired at", (/* @__PURE__ */ new Date()).toISOString());
    try {
      const accessToken = await getPayPalAccessToken(env2);
      const transactions = await getRecentTransactions(accessToken, env2);
      const completedCount = await processTransactions(transactions, env2);
      ...
      console.error("[AUTO-VERIFY] Cron error:", error3);
    }
  }
};
...
var worker_default = {
  ...
  async scheduled(event, env2, ctx) {
    await paypal_ff_auto_verify_default.scheduled(event, env2, ctx);
  },
  async fetch(request, env2, ctx) {
    const url = new URL(request.url);
    ...
    if (path === "/api/paypal/config") {
      return new Response(JSON.stringify({
        clientId: env2.PAYPAL_CLIENT_ID || "",
        currency: "USD"
      }), {
        headers: {
          "Content-Type": "application/json",
    ...
    if (path === "/api/paypal/create-order")
      return paypal_create_order_default.fetch(request, env2, ctx);
    if (path === "/api/paypal/capture-order")
      return paypal_capture_order_default.fetch(request, env2, ctx);
    ...
    if (path === "/api/paypal-ff/test-complete")
      return paypal_ff_test_complete_default.fetch(request, env2, ctx);
    if (path === "/api/paypal-ff/auto-verify")
      return paypal_ff_auto_verify_default.fetch(request, env2, ctx);
    ...
    if (path === "/create-topup-intent")
      return handler2(request, env2);
    if (path === "/get-balance-transactions")
      return handler3(request, env2);
    if (path === "/pay-with-balance")
