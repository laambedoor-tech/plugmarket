...
var _OkResponse = class extends Response {
...
}, OkResponse = _OkResponse;
__publicField(OkResponse, "status", 200);
var _NotFoundResponse = class extends Response {
  constructor(...[body, init]) {
    super(body, {
      ...init,
...
}, NotFoundResponse = _NotFoundResponse;
__publicField(NotFoundResponse, "status", 404);
var NoIntentResponse = class extends NotFoundResponse {
  constructor() {
    super();
  }
}, _MethodNotAllowedResponse = class extends Response {
  constructor(...[body, init]) {
    super(body, {
      ...init,
...
}, MethodNotAllowedResponse = _MethodNotAllowedResponse;
__publicField(MethodNotAllowedResponse, "status", 405);
var _InternalServerErrorResponse = class extends Response {
...
}, InternalServerErrorResponse = _InternalServerErrorResponse;
__publicField(InternalServerErrorResponse, "status", 500);
var _NotModifiedResponse = class extends Response {
...
}, NotModifiedResponse = _NotModifiedResponse;
__publicField(NotModifiedResponse, "status", 304);
var _MovedPermanentlyResponse = class extends Response {
...
}, MovedPermanentlyResponse = _MovedPermanentlyResponse;
__publicField(MovedPermanentlyResponse, "status", 301);
var _FoundResponse = class extends Response {
  constructor(location, init) {
    super(null, {
      ...init,
...
}, FoundResponse = _FoundResponse;
__publicField(FoundResponse, "status", 302);
var _SeeOtherResponse = class extends Response {
  constructor(location, init) {
    super(null, {
      ...init,
...
}, SeeOtherResponse = _SeeOtherResponse;
__publicField(SeeOtherResponse, "status", 303);
var _TemporaryRedirectResponse = class extends Response {
  constructor(location, init) {
    super(null, {
      ...init,
...
}, TemporaryRedirectResponse = _TemporaryRedirectResponse;
__publicField(TemporaryRedirectResponse, "status", 307);
var _PermanentRedirectResponse = class extends Response {
  constructor(location, init) {
    super(null, {
      ...init,
...
}, PermanentRedirectResponse = _PermanentRedirectResponse;
__publicField(PermanentRedirectResponse, "status", 308);
...
function getAssetHeaders(eTag, contentType, cacheStatus, request) {
  let headers = new Headers({
    ETag: `"${eTag}"`
  });
  return contentType !== void 0 && headers.append("Content-Type", contentType), isCacheable(request) && headers.append("Cache-Control", CACHE_CONTROL_BROWSER), headers.append("CF-Cache-Status", cacheStatus), headers;
}
function isCacheable(request) {
  return !request.headers.has("Authorization") && !request.headers.has("Range");
}
function attachCustomHeaders(request, response, configuration) {
  let matches = generateRulesMatcher(
    ...
    ({ set = {}, unset = [] }, replacements) => {
      let replacedSet = {};
      return Object.keys(set).forEach((key) => {
  ...
  return matches.forEach(({ set = {}, unset = [] }) => {
    unset.forEach((key) => {
      ...
      setMap.has(key.toLowerCase()) ? response.headers.append(key, set[key]) : (response.headers.set(key, set[key]), setMap.add(key.toLowerCase()));
    });
  }), response;
}
...
var REDIRECTS_VERSION = 1, HEADERS_VERSION = 2, getResponseOrAssetIntent = async (request, env, configuration, exists) => {
  let url = new URL(request.url), { host, search } = url, { pathname } = url, staticRedirectsMatcher = () => {
  ...
  }, generateRedirectsMatcher = () => generateRulesMatcher(
    configuration.redirects.version === REDIRECTS_VERSION ? configuration.redirects.rules : {},
    ({ status, to }, replacements) => ({
      status,
      to: replacer(to, replacements)
    })
  ), redirectMatch = staticRedirectsMatcher() || generateRedirectsMatcher()({ request })[0], proxied = !1;
  if (redirectMatch)
    if (redirectMatch.status === 200)
      pathname = new URL(redirectMatch.to, request.url).pathname, proxied = !0;
    else {
  ...
  let decodedPathname = decodePath(pathname), intent = await getIntent(decodedPathname, configuration, exists);
  if (!intent) {
  ...
  }), new TemporaryRedirectResponse(encodedDestination + search))) : intent.asset ? intent.asset : env.JAEGER.enterSpan("unknown_action", (span) => (span.setTags({
    pathname,
    status: InternalServerErrorResponse.status
  }), new InternalServerErrorResponse(new Error("Unknown action"))));
}, resolveAssetIntentToResponse = async (assetIntent, request, env, getByETag, analytics) => {
  ...
  }), await getByETag(assetIntent.eTag))), headers = getAssetHeaders(
    assetIntent.eTag,
    asset.contentType,
    asset.cacheStatus,
    request
...
}, canFetch = async (request, env, configuration, exists) => !(await getResponseOrAssetIntent(
  request,
  env,
  {
    ...configuration,
...
var src_default = class extends WorkerEntrypoint {
  ...
  async unstable_getByETag(eTag) {
    let performance = new PerformanceTimer(this.env.UNSAFE_PERFORMANCE), startTime = performance.now(), asset = await getAssetWithMetadataFromKV(
  ...
  async unstable_exists(pathname) {
    ...
    try {
      ...
      analytics.setData({ manifestReadTime: performance.now() - startTimeMs }), analytics.write();
    }
  }
};
